...
from models.models import FileUpload, User, UserInDB, Token, TokenData, \
...
async def get_current_active_user(current_user: User = Depends(get_current_user)):
    ...
    return current_user
@authenticate.get('/ohyeah')
async def  oh_yeah():
    return 'Oh yeah'

@authenticate.post('/login', response_model=Token)
async def get_login_token(user: LoginUser):
    print(user)
...
async def register_user(new_user: User):
    ...
    if (new_user.password is not None) and (new_user.username is not None):
        ...
        raise HTTPException(
            ...
            detail="User failed to supply password or username",
            headers={"WWW-Authenticate": "Bearer"},
        )
